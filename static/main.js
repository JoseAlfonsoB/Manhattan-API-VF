...
// --- LÓGICA CENTRAL ---
async function procesarRuta(puntoInicioOriginal, puntoDestinoOriginal, nombreIni = '', nombreDes = '') {
    ...
    try {
        ...
        // Actualiza nombres de lugares SIEMPRE con reverse geocoding
        await actualizarNombreLugar(puntoInicioOriginal, 'inicio');
        await actualizarNombreLugar(puntoDestinoOriginal, 'destino');
    } catch (error) {
...
function mostrarResultados(distanciaM, duracionS, costoCasetas = 0) {
    ...
    info.textContent = `Datos de la ruta:`;
...
function limpiarTodo() {
  ...
  // Reiniciar los nombres de inicio y destino
  if (typeof nombreInicio !== 'undefined' && nombreInicio) nombreInicio.textContent = "Inicio: ";
  if (typeof nombreDestino !== 'undefined' && nombreDestino) nombreDestino.textContent = "Destino: ";
...
async function actualizarNombreLugar(latlng, tipo) {
    const url = `https://nominatim.openstreetmap.org/reverse?format=json&lat=${latlng.lat}&lon=${latlng.lng}`;
    try {
        const response = await fetch(url);
        const data = await response.json();
        if (tipo === 'inicio' && nombreInicio) nombreInicio.textContent = "Inicio: " + (data.display_name || `${latlng.lat.toFixed(5)}, ${latlng.lng.toFixed(5)}`);
        if (tipo === 'destino' && nombreDestino) nombreDestino.textContent = "Destino: " + (data.display_name || `${latlng.lat.toFixed(5)}, ${latlng.lng.toFixed(5)}`);
    } catch {
        if (tipo === 'inicio' && nombreInicio) nombreInicio.textContent = "Inicio: " + `${latlng.lat.toFixed(5)}, ${latlng.lng.toFixed(5)}`;
        if (tipo === 'destino' && nombreDestino) nombreDestino.textContent = "Destino: " + `${latlng.lat.toFixed(5)}, ${latlng.lng.toFixed(5)}`;
    }
}
